...
Pandas < .18.1 has a bug where surveys_df['weight'].describe() may return a runtime error. This refers to the old ecology data, not clear if it applies to the eebo data. 
...
Questions seem to arise around queries in the form of `eebo_df[eebo_df[...]], i.e. compound indexing in a single line. It may be helpful to explain the order in which these statements are evaluated by splitting them into multiple lines. 
...
* What happens if there is no data for a year?
	...
	Nothing really. Since we are explicitly looping over the years where there _are_ data, this is a deliberately out-of-context question - but it's useful to see whether people recognise that, or get confused. 
